...
# Создаем папку secrets если не существует (для счетчиков, метрик, VK.ОРД данных)
...
async def manual_pnc_start(message: Message, state: FSMContext):
    """
    ВРЕМЕННАЯ ЗАГЛУШКА для кнопки «➕  Добавить вручную».
    Реальная логика ещё в разработке.
    """
    await message.answer(
        "Приношу свои глубокие извинения, я еще в разработке этого пункта..",
        reply_markup=invoice_actions_kb()
    )
...
async def main() -> None:
    ...
    try:
        ...
        dp.message.register(vk_ord_add_creative, F.text.lower().contains('креатив'))
                        ...
                        # Справочник ККТУ
        dp.message.register(vk_ord_kktu_show, F.text.lower().contains('справочник ккту'))
        dp.message.register(vk_ord_kktu_show, match_contains("справочник ккту"))
        dp.message.register(vk_ord_kktu_prev, match_contains("пред. страница"))
        dp.message.register(vk_ord_kktu_next, match_contains("след. страница"))
        dp.message.register(vk_ord_kktu_back_to_menu, match_contains("vk.орд меню"))
...
# шаги мастера VK.ОРД — контрагент
        ...
        dp.message.register(vk_ord_creative_type_step,   StateFilter("vk_ord_creative_type"))
...
# ---------- СПРАВОЧНИК ККТУ (КАТЕГОРИИ ТОВАРОВ И УСЛУГ) ----------

# Список кодов 3-го уровня ККТУ.
# При необходимости дополни его всеми 369 кодами из официального файла.
KKTU_CODES: list[str] = [
    "1.1.1 ВИНО",
    ...
    "5.2.9 ИНФОРМАЦИОННО-РАЗВЛЕКАТЕЛЬНЫЕ УСЛУГИ СОТОВОЙ",
    ...
    "21.1.2 ПОМОЩЬ ПРИ СТИХИЙНЫХ БЕДСТВИЯХ, КАТАСТРОФАХ,",
    "21.1.3 ПОМОЩЬ В ПРЕДОТВРАЩЕНИИ НЕСЧАСТНЫХ СЛУЧАЕВ,",
    ...
    "21.4.7 ФОРМИРОВАНИЕ НЕТЕРПИМОСТИ К НАРУШЕНИЯМ",
    "21.4.8 ЗАЩИТА ТРАДИЦИЙ, КУЛЬТУРЫ, ЯЗЫКОВ НАРОДОВ РФ",
    "21.4.9 ПРОВЕДЕНИЕ ПОИСКОВОЙ РАБОТЫ (НЕИЗВЕСТНЫЕ",
    ...
    "28.1.2 МАГАЗИНЫ АВТО- МОТОТЕХНИКИ, ЗАПЧАСТЕЙ И",
...
KKTU_PAGE_SIZE = 15  # по умолчанию показываем до 15 кодов на странице
  # по умолчанию показываем до 20 кодов на странице


def _get_kktu_page(user_id: str) -> int:
    """
    Возвращает сохранённый номер страницы справочника ККТУ для пользователя.
    Сохраняем его в общем VK_ORD_STATE_FILE.
    """
    st = _get_user_state(user_id)
    try:
        page = int(st.get("kktu_page") or 1)
    except Exception:
        page = 1
    if page < 1:
        page = 1
    total_pages = max(1, (len(KKTU_CODES) + KKTU_PAGE_SIZE - 1) // KKTU_PAGE_SIZE)
    if page > total_pages:
        page = total_pages
    return page


def _set_kktu_page(user_id: str, page: int) -> None:
    st = _get_user_state(user_id)
    st["kktu_page"] = int(max(1, page))
    _set_user_state(user_id, st)


def _build_kktu_page_text(page: int) -> tuple[str, int, int]:
    """
    Собирает текст для страницы справочника ККТУ.
    Возвращает (text, page, total_pages).
    """
    total_items = len(KKTU_CODES)
    if total_items == 0:
        return "Справочник ККТУ пока пуст. Позже здесь появятся коды категорий.", 1, 1

    total_pages = (total_items + KKTU_PAGE_SIZE - 1) // KKTU_PAGE_SIZE
    if page < 1:
        page = 1
    if page > total_pages:
        page = total_pages

    start = (page - 1) * KKTU_PAGE_SIZE
    end = min(start + KKTU_PAGE_SIZE, total_items)
    items = KKTU_CODES[start:end]

    # Заголовок страницы + аккуратный маркдаун-список
    lines: list[str] = [f"*Справочник ККТУ* (стр. {page}/{total_pages})", ""]
    for item in items:
        code, title = (item.split(" ", 1) + [""])[:2]
        code = code.strip()
        title = title.strip()
        if title:
...
def vk_ord_kktu_kb(page: int, total_pages: int) -> _ReplyKeyboardMarkup_vk:
    """
    Клавиатура навигации по справочнику ККТУ.
    """
    nav_row: list[_KeyboardButton_vk] = []
    if page > 1:
        nav_row.append(_KeyboardButton_vk(text="◀️ Пред. страница"))
    if page < total_pages:
        nav_row.append(_KeyboardButton_vk(text=" След. страница ➡️"))

    if not nav_row:
        nav_row.append(_KeyboardButton_vk(text="Обновить страницу"))

    return _ReplyKeyboardMarkup_vk(
        keyboard=[
            nav_row,
            [_KeyboardButton_vk(text="🔙 В VK.ОРД меню")],
        ],
        resize_keyboard=True,
    )


async def vk_ord_kktu_show(message: _Message_vk, state: _FSMContext_vk):
    """
    Старт показа справочника ККТУ из меню VK.ОРД.
    Всегда открывает первую страницу.
    """
    user_id = str(message.from_user.id)
    page = 1
    text, page, total_pages = _build_kktu_page_text(page)
    _set_kktu_page(user_id, page)
    await message.answer(text, reply_markup=vk_ord_kktu_kb(page, total_pages))


async def vk_ord_kktu_prev(message: _Message_vk, state: _FSMContext_vk):
    """
    Переход на предыдущую страницу справочника ККТУ.
    """
    user_id = str(message.from_user.id)
    current_page = _get_kktu_page(user_id)
    new_page = max(1, current_page - 1)
    text, page, total_pages = _build_kktu_page_text(new_page)
    _set_kktu_page(user_id, page)
    await message.answer(text, reply_markup=vk_ord_kktu_kb(page, total_pages))


async def vk_ord_kktu_next(message: _Message_vk, state: _FSMContext_vk):
    """
    Переход на следующую страницу справочника ККТУ.
    """
    user_id = str(message.from_user.id)
    current_page = _get_kktu_page(user_id)
...
async def vk_ord_kktu_back_to_menu(message: _Message_vk, state: _FSMContext_vk):
    """
    Возврат из справочника ККТУ в меню VK.ОРД.
    """
    await message.answer("Возвращаю в меню VK.ОРД.", reply_markup=vk_ord_menu_kb())
...
def vk_ord_menu_kb() -> _ReplyKeyboardMarkup_vk:
    return _ReplyKeyboardMarkup_vk(
        keyboard=[
            ...
            [_KeyboardButton_vk(text="Оформить креатив (ERID)")],
            [_KeyboardButton_vk(text="Справочник ККТУ"), _KeyboardButton_vk(text="В главное меню")],
        ],
        resize_keyboard=True,
    )
...
VK_ORD_CREATIVE_TYPES: list[str] = [
    "Аудио",
    "Видео",
    "Баннер",
    "Текст",
...
def vk_ord_creative_type_kb() -> _ReplyKeyboardMarkup_vk:
    """
    Клавиатура выбора ТИПа креатива строго из перечня VK.ОРД.
    Наверху — типы креатива, внизу — стандартные кнопки навигации.
    """
    rows: list[list[_KeyboardButton_vk]] = [
        [
            _KeyboardButton_vk(text="Аудио"),
            _KeyboardButton_vk(text="Видео"),
            _KeyboardButton_vk(text="Баннер"),
        ],
        [
            _KeyboardButton_vk(text="Текст"),
            _KeyboardButton_vk(text="Аудио эфир"),
            _KeyboardButton_vk(text="Видео эфир"),
        ],
        [
            _KeyboardButton_vk(text="Текстово-графический блок"),
        ],
        [
            _KeyboardButton_vk(text="Текстовый блок с видео"),
            _KeyboardButton_vk(text="Текстово-графический блок с видео"),
        ],
        [
            _KeyboardButton_vk(text="Текстовый блок с аудио"),
            _KeyboardButton_vk(text="Текстово-графический блок с аудио"),
        ],
        [
            _KeyboardButton_vk(text="Текстовый блок с аудио и видео"),
        ],
        [
            _KeyboardButton_vk(text="Текстово-графический блок с аудио и видео"),
        ],
        [
            _KeyboardButton_vk(text="HTML5-баннер"),
        ],
        [
    ...
    return _ReplyKeyboardMarkup_vk(keyboard=rows, resize_keyboard=True)
...
async def vk_ord_creative_media_step(message: _Message_vk, state: _FSMContext_vk):
    ...
    После успешного ввода/загрузки переходим к шагу 6 (выбор типа креатива).
    """
    ...
    if media_info is not None:
        ...
        if not ok:
            # Ошибка загрузки — остаёмся на этом же шаге
            await message.answer(
        ...
        # Дополнительно фиксируем исходное имя файла и его расширение
        base_name = filename or "файл"
        name_only, ext = os.path.splitext(base_name)
        await state.update_data(
            vk_ord_creative_media_raw=external_id,
            vk_ord_creative_media_filename=name_only,
            vk_ord_creative_media_ext=ext.lstrip("."),
        )

    else:
        # 2. Если медиа нет — пользователь вводит external_id(ы) руками
        media_raw = (message.text or "").strip()
        if not media_raw:
            await message.answer(
                "Отправьте картинку/видео/файл с креативом одним сообщением — "
                "бот автоматически подгрузит медиа в VK.ОРД и прикрепит к данному креативу.\n"
                "Либо укажите `external_id` уже загруженного в VK.ОРД медиафайла.",
                reply_markup=step_kb(),
                parse_mode="Markdown",
            )
            await state.set_state("vk_ord_creative_media")
    ...
    # 3. Переходим к шагу выбора типа креатива
    await state.set_state("vk_ord_creative_type")
    await message.answer(
        "🎨 *Оформление креатива (шаг 6/7)*\n\n"
        "Выберите ТИП креатива из списка ниже.",
        reply_markup=vk_ord_creative_type_kb(),
        parse_mode="Markdown",
    )


async def vk_ord_creative_type_step(message: _Message_vk, state: _FSMContext_vk):
    """
    Шаг 6/7: выбор ТИПа креатива из фиксированного списка VK.ОРД.
    Любой текст, не совпадающий с разрешёнными значениями, не принимаем.
    """
    creative_type = (message.text or "").strip()

    # Навигация
    if creative_type == "◀  Назад":
        ...
        return
    if creative_type == "✖  На главную":
        await state.clear()
        await message.answer("Возвращаю в главное меню.", reply_markup=main_kb())
        return

    # Жёсткая проверка на допустимые типы
    if creative_type not in VK_ORD_CREATIVE_TYPES:
        await message.answer(
            "Пожалуйста, выберите ТИП креатива, используя кнопки на клавиатуре ниже.",
            reply_markup=vk_ord_creative_type_kb(),
        )
        await state.set_state("vk_ord_creative_type")
        return

    # Сохраняем выбранный тип и переходим к ККТУ
    await state.update_data(vk_ord_creative_type=creative_type)
    await state.set_state("vk_ord_creative_kktu")
    await message.answer(
        "🎨 *Оформление креатива (шаг 7/7)*\n\n"
...
async def vk_ord_creative_kktu_step(message: _Message_vk, state: _FSMContext_vk):
    ...
    media_filename = (data.get("vk_ord_creative_media_filename") or "").strip()
    media_ext = (data.get("vk_ord_creative_media_ext") or "").strip()

    if media_filename or media_ext:
        media_desc = media_filename or "файл"
        if media_ext:
            media_desc += f".{media_ext}"
    elif media_raw:
        media_desc = media_raw
    else:
        media_desc = "не указаны"

    # Экранируем пользовательские данные для безопасного Markdown
    name_preview = md_escape((data.get("vk_ord_creative_name") or "").strip())
    url_preview = md_escape((data.get("vk_ord_creative_url") or "").strip())
    period_preview = md_escape((data.get("vk_ord_creative_period") or "").strip())
    texts_preview_safe = md_escape(texts_preview or "не заданы")
    media_desc_safe = md_escape(media_desc)
    kktu_preview = md_escape(kktu_raw or "не указаны")

    text = (
        "Проверьте данные креатива:\n"
        f"• Название: *{name_preview}*\n"
        f"• URL: `{url_preview}`\n"
        f"• Период: {period_preview}\n"
        f"• Тексты: {texts_preview_safe}\n"
        f"• Медиафайл: {media_desc_safe}\n"
        f"• ККТУ: {kktu_preview}\n\n"
        "Если всё верно — нажмите «✅ Подтвердить»."
    )
    kb = _ReplyKeyboardMarkup_vk(
...
# Используем OUTPUT_DIR из config, если не задан - используем относительный путь
GENERATED_PATH = getattr(config, 'OUTPUT_DIR', 'generated')
...
def build_inn_summary_from_paragraphs(paragraphs, file_path: str, inn: str) -> str:
    ...
    for p in paragraphs:
        ...
        if t:
            lines.append(t)
    if not lines:
        return f"Документ: {_os_inn.path.basename(file_path)}\nИНН: {inn}"
    ...
    if not header:
        for ln in lines:
            ...
            if "договор" in low and "№" in ln:
                ...
                break
    if not header:
        header = f"Документ: {_os_inn.path.basename(file_path)}"
